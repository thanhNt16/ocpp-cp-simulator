...
export class OCPPMessageHandler {
  ...
  public sendMeterValue(
    ...
    const messageId = this.generateMessageId();
    let payload: request.MeterValuesRequest;

    if (typeof meterValue === 'number') {
      // Simple case - just a meter value
      payload = {
      ...
      // Complex case - detailed sampledValue array
      payload = {
        transactionId: transactionId,
        connectorId: connectorId,
        meterValue: [
          {
            timestamp: meterValue.timestamp || new Date().toISOString(),
            sampledValue: meterValue.sampledValue,
          },
        ],
      };
    }
